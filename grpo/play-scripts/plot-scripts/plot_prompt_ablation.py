#!/usr/bin/env python3
"""
Plot prompt template ablation results for GRPO experiments, fetched from W&B.
...
Two runs comparing R1-Zero prompt vs question-only prompt over 200 GRPO steps.
"""
...
RUN_NAMES = [
    "off_policy_full_e1_tb256_ga64",  # R1-Zero prompt (think/answer structure)
    "question_only_e1_tb256_ga64",    # Question-only prompt
]

DISPLAY_NAMES = {
    "off_policy_full_e1_tb256_ga64": "R1-Zero prompt",
    "question_only_e1_tb256_ga64": "Question-only prompt",
}
...
COLORS = {
    ...
    "question_only_e1_tb256_ga64": "#56B4E9",    # Sky blue
}
...
def save_csv(all_data: dict[str, dict[str, pd.DataFrame]], output_dir: Path) -> None:
    ...
    csv_path = output_dir / "prompt_ablation_data.csv"
...
def plot_results(
    ...
    fig.suptitle(
        "Prompt template ablation: R1-Zero vs Question-only",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
...
def main():
    output_dir = Path(__file__).resolve().parent.parent.parent / "results" / "prompt_ablation"
    ...
    plot_results(all_data, output_dir / "prompt_ablation.png")

#!/usr/bin/env python3
"""
Plot length normalization ablation results for GRPO experiments, fetched from W&B.
...
RUN_NAMES = ["len_norm_mean", "len_norm_constant", "len_norm_microbatch"]

DISPLAY_NAMES = {
    "len_norm_mean": "÷ response length",
    "len_norm_constant": "÷ max_gen_len (1024)",
    "len_norm_microbatch": "÷ microbatch max response",
}

COLORS = {
    "len_norm_mean": "#E69F00",        # Orange
    "len_norm_constant": "#56B4E9",    # Sky blue
    "len_norm_microbatch": "#009E73",  # Bluish green
}
...
LINE_WIDTHS = {
    "len_norm_mean": 2,
    "len_norm_constant": 1.5,
    "len_norm_microbatch": 1.5,
}
...
def save_csv(all_data: dict[str, dict[str, pd.DataFrame]], output_dir: Path) -> None:
    ...
    pd.DataFrame(rows).to_csv(output_dir / "length_normalization_data.csv", index=False)
    print(f"Saved CSV to {output_dir / 'length_normalization_data.csv'}")
...
def plot_results(
    ...
    fig.suptitle(
        "Length normalization ablations: GRPO experiments",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
    ...
    for ax, (metric_key, meta) in zip(axes, METRICS.items()):
        for run_name in RUN_NAMES:
            ...
            linestyle = LINE_STYLES[run_name]
            if meta["markersize"] > 0:
                ax.plot(
                    df["step"],
                    df["value"],
                    marker="o",
...
def main():
    output_dir = Path(__file__).resolve().parent.parent.parent / "results" / "length_normalization"
    ...
    plot_results(all_data, output_dir / "length_normalization.png")

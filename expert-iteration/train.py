...
# Training hyperparameters (defaults - override with command-line args)
...
batch_per_ei_default = 1024
num_rollouts_default = 2
batch_per_ei = 1024
num_rollouts = 2 # number of outputs to generate for each example
max_train_steps = 75
eval_steps_interval = 25
wandb_run_name = f"run_S{max_train_steps}_D{batch_per_ei}_R{num_rollouts}"
...
eval_log_dict = {
            ...
            "eval/avg_acc": vllm_eval_results['accuracy']['avg_acc'],
            "eval/avg_format_acc": vllm_eval_results['accuracy']['avg_format_acc'],
            "eval/avg_correct_res_len": avg_correct_res_len,
            "eval/avg_incorrect_res_len": avg_incorrect_res_len,
            "eval/dt": dt
        }
...
while True:
    ...
    for step in range(max_steps):
        ...
        if train_step_counter % eval_steps_interval == 0 and run_intermediate_eval:
            ...
            ckpt_dir = output_dir + f"/checkpoint_step_{train_step_counter}"
        ...
        if train_step_counter >= max_train_steps:
            break
    
    if train_step_counter >= max_train_steps:
        break

    # update the model in vllm model
    pretty_print(f"Updating the model in vLLM model...", title="vLLM model updating")
    load_policy_into_vllm_instance(model, vllm_model)

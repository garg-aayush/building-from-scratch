...
# for logging
import wandb
from helper_scripts.hellaswag import iterate_examples, render_example
from helper_scripts.model import GPT, GPTConfig
...
# params
# -------------------------------------------------------------#
wandb_project = "bpe-tests" # wandb project name
wandb_run_name = "gpt2-bpe-fineweb-16K" # wandb run name
data_root = "/workspace/shards_fineweb_16K" # data root
ckpt_dir = "/workspace/ckpt" # checkpoint directory
eval_interval = 200      # (steps) interval for validation and hellaSwag evaluation
...
max_steps = 5000       # total number of steps, FineWeb-Edu 10B tokens (1 epoch training 3B/ 2^19)
weight_decay = 0.1      # weight decay for optimizer
betas = (0.9, 0.95)     # betas for optimizer
# model
vocab_size = 16448     # vocabulary size 16128 + 256 byte pieces + 1 <endoftext> token -> 16385 nice number: 16448
...
class ValDataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes, split, data_root='shards', seed=42):
        ...
        self.eot = enc.special_tokens["<|endoftext|>"]  # end of text token
        print("Special tokens: ", enc.special_tokens, "EOT: ", self.eot)
...
enc = BpeTokenizer()
enc.load("data/fineweb-000_00003-gpt4-train-16384.model")
...
# Pre-calculate once at the beginning
def calculate_val_set_stats(val_loader, tokenizer, val_loss_steps):
    """Calculate total tokens and bytes for the validation batches we'll use"""
    val_loader.reset()
    total_tokens = 0
    total_bytes = 0
    ...
    for _ in range(val_loss_steps):
        ...
        # Decode tokens back to text to count bytes
        # (or if you saved the original text, use that)
        for batch_idx in range(x.size(0)):
            text = tokenizer.decode(x[batch_idx].tolist())
            total_bytes += len(text.encode('utf-8'))
    
    return total_tokens, total_bytes

# Calculate once
VAL_TOKENS_PER_EVAL, VAL_BYTES_PER_EVAL = calculate_val_set_stats(val_loader, enc, val_loss_steps)
print(f"VAL_TOKENS_PER_EVAL: {VAL_TOKENS_PER_EVAL}, VAL_BYTES_PER_EVAL: {VAL_BYTES_PER_EVAL}", f"Compression ratio: {VAL_BYTES_PER_EVAL / VAL_TOKENS_PER_EVAL:.2f}")
...
def estimate_loss():
    ...
    # get bpb (bits per byte)
    avg_val_loss = val_loss_accum.item() 
    bpb = (avg_val_loss * VAL_TOKENS_PER_EVAL * np.log2(np.e)) / VAL_BYTES_PER_EVAL
    model.train()
    return avg_val_loss, bpb
...
def generate_samples():
    ...
    # prefix the tokens
    tokens = enc.encode(start_seq, 'all')  # 8 tokens
...
for step in range(max_steps):
    ...
    if (step % eval_interval == 0 or last_step) and run_validation:
        val_loss, bpb = estimate_loss()
        if master_process:
            print(f"validation loss: {val_loss:.4f}")
            log_dict["val/loss"] = val_loss
            log_dict["val/bpb"] = bpb
            ...
            #         "config": config
            #     }
            #     print(f"Saving model at step {step}, bpb: {bpb:.4f}, val_loss: {val_loss:.4f} -> best_val_loss: {best_val_loss:.4f}")
